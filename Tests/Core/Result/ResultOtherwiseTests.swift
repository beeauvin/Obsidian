...
struct ResultOtherwiseTests {
  ...
  func otherwise_error_provider_evaluates_with_error_and_returns_provider_result_when_failure() throws {
    // Given
    let result: Result<Int, TestError> = .failure(.expected)
  ...
  @Test("otherwise(async_error_provider:) awaits with error and returns provider result when failure")
  func otherwise_async_error_provider_awaits_with_error_and_returns_provider_result_when_failure() async throws {
    // Given
    let result: Result<Double, TestError> = .failure(.expected)
