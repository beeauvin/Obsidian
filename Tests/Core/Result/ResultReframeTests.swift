...
@Suite("Obsidian/Foundation/Result: Reframe")
struct ResultReframeTests {
  ...
  private func async_error_result_transformer(_ error: TestError) async -> Result<String, OtherError> {
    return error == .expected ? .success("Recovered") : .failure(.different)
  }
  ...
  @Test("reframe(_:) with async result returning transformer returns success unchanged when success")
