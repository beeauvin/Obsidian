...
public extension Result {
  ...
  func recover(_ recovery: (Failure) -> Success?) -> Result<Success, Failure> {
    switch self {
      case .success:
        return self
  ...
  func recover<NewFailure>(_ recovery: (Failure) -> Result<Success, NewFailure>) -> Result<Success, NewFailure> {
    switch self {
  ...
  /// - Returns: The original success, a recovered success, or the original failure if recovery failed
  func recover(_ recovery: (Failure) async -> Success?) async -> Result<Success, Failure> {
    switch self {
      case .success:
        return self
      case .failure(let error):
  ...
  /// - Returns: The original success or the result of the recovery attempt
  func recover<NewFailure>(_ recovery: (Failure) async -> Result<Success, NewFailure>) async -> Result<Success, NewFailure> {
    switch self {
      case .success(let value):
        return .success(value)
      case .failure(let error):
