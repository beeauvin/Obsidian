...
public extension Optional {
  ...
  func when(something action: (Wrapped) -> Void) {
    if let wrapped = self {
      action(wrapped)
    }
  }
  ...
  func when(something action: (Wrapped) async -> Void) async {
    if let wrapped = self {
  ...
  func when(nothing action: () async -> Void) async {
    if self == nil {
      await action()
    }
  }
